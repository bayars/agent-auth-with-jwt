...
Four standalone FastMCP tool servers, each with its own Docker image, Helm chart, and build script.
...
```bash
# From repo root — builds and starts all four tool servers
docker compose up mcp-http-caller mcp-file-generator mcp-text-generator mcp-text-utils -d
...
```
tools/
├── build-all.sh                   ← build all four images
├── http-caller/
...
├── mcp-tools-stack/               ← umbrella Helm chart (deploys all four)
│   ├── Chart.yaml
│   ├── values.yaml
